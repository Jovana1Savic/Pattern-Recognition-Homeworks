...
% value of probability density functions in the sample diagram. 
...
% Find points which have same probability denstity function value and plot
% them.
figure(1);
hold on
fmax = max(max(f1));
...
figure(1);
hold on
contour(x,y,h',[0 0],'k','LineWidth',1); % Plot Bayesian classifier.
contour(x,y,h', [-log(mu(4)) -log(mu(4))], 'g', 'LineWidth', 1); % Plot Neyman-Pearson's classifier. 
contour(x,y,h', [-log(mu(5)) -log(mu(5))], 'm', 'LineWidth', 1); % Plot Neyman-Pearson's classifier. 
legend({'class1', 'class2', 'class1 const', 'class2 const', 'Bayesian', ... 
    ['$\mu = ', num2str(mu(4)), '$'], ...
    ['$\mu = ', num2str(mu(5)), '$']}, 'Interpreter','latex');
...
[eps1, eps2] = findIntegralErrors(x, y, f1, f2, h, -log(mu(4))); % Neyman-Pearson.
disp(['Neyman-Person classifer errors mu = ', num2str(mu(4)), ...
    ' , eps0 = ', num2str(eps0(4))]);
display(eps1);
display(eps2);
...
eps1 = sum(fh1(h1>0)); eps2 = sum(fh2(h2<=0));
disp('Bayesian classifer errors based on f(h)');
display(eps1);
display(eps2);

eps1 = sum(fh1(h1>-log(mu(4)))); eps2 = sum(fh2(h2<=-log(mu(4))));
disp(['Neyman-Person classifer errors mu = ', num2str(mu(4)), ...
    ' , eps0 = ', num2str(eps0(4)), ' based on f(h)']);
display(eps1);
display(eps2);

eps1 = sum(fh1(h1>-log(mu(5)))); eps2 = sum(fh2(h2<=-log(mu(5))));
disp(['Neyman-Person classifer errors mu = ', num2str(mu(5)), ...
...
%   scale. 
    ...
    % Find cumulative sum F(h) = eps0 and mu = -log(h). 
    sum = 0;
    for i=1:size(fh,2)
        sum = sum + fh(i);
        eps0(i) = sum;
...
%   discriminatory function. 
    ...
    % Find and plot histgram. 
    figure,
    fh_hist = histogram(h, 'Normalization', 'probability'); 
