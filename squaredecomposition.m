...
% Find criteria function for each number of classes and the maximum
% difference between any two probabilities. 
% Plot both in order to try and conclude the original number of classes. 
...
for L=2:10 % Don't go over 10 classes at worst.
    ...
    % Try once. 
    [omega, l] = squareDecomposition(X, L);
    J(L) = minimizationFunction(X, L, omega);
    P(L) = diffProbabilities(X,L,omega);
    
    % Try again. 
    [omega, l] = squareDecomposition(X, L);
    J(L) = (J(L) + minimizationFunction(X, L, omega))/2;
    P(L) = (P(L) + diffProbabilities(X,L,omega))/2;
end

% Plot results. 
figure, plot(J), title('J(L)');
