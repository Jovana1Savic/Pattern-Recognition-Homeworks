...
for i=1:N
    for j=1:N
        ...
        for k=1:M
            % Test first class.
            [m, error] = performWald(eps1, eps2, 1,M1, S1, M21, S21, M22, S22, P21);
            m1 = m1 + m; 
            error1 = error1 + error;
            %Test second class.
            [m, error] = performWald(eps1, eps2, 2,M1, S1, M21, S21, M22, S22, P21);
            m2 = m2 + m; 
            error2 = error2 + error;
        end
...
title(['First class - $\varepsilon_1=',num2str(eps(50)), '$'], 'Interpreter','latex');
xlabel('$\varepsilon_2$', 'Interpreter', 'latex');

second_class = second_class_avg(50,:);
figure(4), semilogx(eps, second_class);
title(['Second class - $\varepsilon_1=',num2str(eps(50)),'$'],'Interpreter','latex');
xlabel('$\varepsilon_2$', 'Interpreter', 'latex');
...
for i = 1:N
    X1(:, i) = getFirstClassData(M1, S1);
end

% Generate second class' data points.
for i = 1:N
    X2(:, i) = getSecondClassData(M21, S21, M22, S22, P21);
end
...
for i=1:N
    X = [X1(1, i); X1(2, i)];   
    h1(i) = H(X, M1, S1, M21, S21, M22, S22, P21); 
end

% Find h(X)/omega2.
for i=1:N
    X = [X2(1, i); X2(2, i)];
    h2(i) = H(X, M1, S1, M21, S21, M22, S22, P21); 
end

% Plot histograms.
figure(5),
histogram(h1, 'Normalization', 'probability');
title('$f(h/ \omega_1)$', 'Interpreter', 'latex');

figure(6),
histogram(h2, 'Normalization', 'probability');
title('$f(h/ \omega_2)$', 'Interpreter', 'latex');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Find expected number of measurements needed when eps1 = const. 
...
for i=1:size(eps,2)
    ...
    %Eps1 const for second class. 
    [a b] = WaldsTestLimits(eps1, eps2); 
    m2(i) = (b*(1-eps2)+a*eps2)/eta2;
end

% Plot dependencies in log scale.
figure (7), semilogx(eps, m1);
title(['First class - $\varepsilon_1=',num2str(eps(50)),'$'],'Interpreter','latex ');
hold on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Find expected number of measurements needed when eps2 = const. 

for i=1:size(eps,2)
    % Eps1 const for first class.
    eps2 = eps(50); eps1 = eps(i);
    [a b] = WaldsTestLimits(eps1, eps2); 
    m1(i) = (a*(1-eps1)+b*eps1)/eta1;
...
% Plot dependencies in log scale.
figure (9), semilogx(eps, m1);
title(['First class - $\varepsilon_2=',num2str(eps(50)),'$'],'Interpreter','latex ');
hold on
semilogx(eps, first_class_avg(:,50));
legend('Theoretical', 'Actual');
...
figure(10), semilogx(eps, m2);
title(['Second class - $\varepsilon_2=',num2str(eps(50)),'$'],'Interpreter','latex ');
hold on
semilogx(eps, second_class_avg(:,50));
legend('Theoretical', 'Actual');
xlabel('$\varepsilon_1$', 'Interpreter', 'latex');
...
function X = getFirstClassData(M1, S1)
...
function X = getSecondClassData(M21, S21, M22, S22, P21)
...
function [m, error] = performWald(eps1, eps2, class, M1, S1, M21, S21, M22, S22, P21)
...
% before reaching conclusion. Sets error to 1 if there was an error.
    ...
    while (1)       
        % Generate sample. 
        if class == 1
            X = getFirstClassData(M1, S1);
        else 
            X = getSecondClassData(M21, S21, M22, S22, P21);
        end
        m = m + 1;
        sm = sm + H(X,M1, S1, M21, S21, M22, S22, P21);
